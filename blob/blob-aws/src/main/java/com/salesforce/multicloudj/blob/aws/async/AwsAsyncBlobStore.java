...
public class AwsAsyncBlobStore extends AbstractAsyncBlobStore implements AwsSdkService {
  ...
  private CompletableFuture<UploadResponse> doUpload(
  ...
  protected CompletableFuture<Void> doDelete(String key, String versionId) {
    ...
    return client.deleteObject(aws).thenAccept(response -> {});
  }
  ...
  protected CompletableFuture<Void> doList(
    ...
    // the publisher lets us subscribe to pagination events automatically, so we don't need to
    // invoke
    // each pages fetch operation
    ListObjectsV2Publisher publisher = client.listObjectsV2Paginator(aws);
    // here we wrap our consumer with a translation layer, so we can convert to the appropriate
    // type.
    ConsumerWrapper<ListObjectsV2Response, ListBlobsBatch> wrapper =
        new ConsumerWrapper<>(consumer, transformer::toBatch);
  ...
  protected CompletableFuture<MultipartUpload> doInitiateMultipartUpload(
    ...
    return client
        .createMultipartUpload(transformer.toCreateMultipartUploadRequest(request))
        ...
        .thenApply(
            response ->
                new UploadPartResponse(
                    mpp.getPartNumber(), response.eTag(), mpp.getContentLength()));
  }
  ...
  protected CompletableFuture<MultipartUploadResponse> doCompleteMultipartUpload(
    ...
    return client
        .completeMultipartUpload(transformer.toCompleteMultipartUploadRequest(mpu, parts))
  ...
  protected CompletableFuture<List<UploadPartResponse>> doListMultipartUpload(MultipartUpload mpu) {
    return client
        .listParts(transformer.toListPartsRequest(mpu))
        .thenApply(
            response ->
                response.parts().stream()
                    ...
                    .map(
                        (part) ->
                            new UploadPartResponse(part.partNumber(), part.eTag(), part.size()))
                    .collect(Collectors.toList()));
  }
  ...
  protected CompletableFuture<Void> doAbortMultipartUpload(MultipartUpload mpu) {
    return client
        .abortMultipartUpload(transformer.toAbortMultipartUploadRequest(mpu))
        .thenAccept(response -> {});
  }
  ...
  protected CompletableFuture<Map<String, String>> doGetTags(String key) {
    return client
        .getObjectTagging(transformer.toGetObjectTaggingRequest(key))
        .thenApply(
            response -> response.tagSet().stream().collect(Collectors.toMap(Tag::key, Tag::value)));
  }

  @Override
  protected CompletableFuture<Void> doSetTags(String key, Map<String, String> tags) {
    return client
        .putObjectTagging(transformer.toPutObjectTaggingRequest(key, tags))
        .thenAccept(response -> {});
  }
    ...
    if (useTransferListener) {
      builder.downloadFileRequestTransformer(
          request -> {
            ...
            request.addTransferListener(new InternalS3LoggingTransferListener());
          });
    }
  ...
  protected CompletableFuture<DirectoryUploadResponse> doUploadDirectory(
    ...
    software.amazon.awssdk.transfer.s3.model.UploadDirectoryRequest uploadDirectoryRequest =
        transformer.toUploadDirectoryRequest(
            ...
            useTransferListener,
            useTransferListener ? new InternalS3LoggingTransferListener() : null);
    return transferManager
  ...
  private static class InternalS3LoggingTransferListener implements TransferListener {
    @Override
    public void transferInitiated(Context.TransferInitiated context) {
      logger.debug("S3 directory transfer initiated: {}", context);
    }

    @Override
    public void bytesTransferred(Context.BytesTransferred context) {
      logger.trace("S3 directory bytes transferred: {}", context.progressSnapshot());
    }

    @Override
    public void transferComplete(Context.TransferComplete context) {
      long bytes = context.progressSnapshot().transferredBytes();
      logger.debug("S3 directory transfer complete, bytes={}", bytes);
    }

    @Override
    public void transferFailed(Context.TransferFailed context) {
      long bytes = context.progressSnapshot().transferredBytes();
      logger.warn("S3 directory transfer failed, bytes={}", bytes, context.exception());
    }
  }
  ...
  public static class Builder extends AsyncBlobStoreProvider.Builder {
    ...
    private static void applyCommonConfig(
        S3AsyncClientBuilder builder, Builder config, Region regionObj) {
        ...
        if (config.getProxyEndpoint() != null
          ...
          if (config.getUseEnvironmentVariableProxyValues() != null) {
            proxyConfigBuilder.useEnvironmentVariableValues(
                config.getUseEnvironmentVariableProxyValues());
          }
    ...
    private static void applyCommonConfig(
      ...
      if (config.getRetryConfig() != null) {
        builder.retryConfiguration(
            retryConfig -> retryConfig.numRetries(config.getRetryConfig().getMaxAttempts() - 1));
      }
    ...
    public AsyncBlobStore build() {
      ...
      if (tm == null) {
        ...
        if (getTransferDirectoryMaxConcurrency() != null) {
          transferManagerBuilder.transferDirectoryMaxConcurrency(
              getTransferDirectoryMaxConcurrency());
        }
