...
/**
 * This interface defines the providing common (i.e. substrate-agnostic) functionality for blob
 * store
 */
public interface BlobStore extends SdkService, Provider {
  ...
  /**
   * Performs args validation and passes the call to substrate-specific upload method Note:
   * Specifying the contentLength in the UploadRequest can dramatically improve upload efficiency
  ...
  /**
   * Performs validation and invokes substrate-specific copyFrom method. Copies a blob from a source
   * bucket to the current bucket.
   *
   ...
   */
  MultipartUploadResponse completeMultipartUpload(
   ...
   * @throws com.salesforce.multicloudj.common.exceptions.ResourceNotFoundException if the bucket
   *     does not exist
   * @throws com.salesforce.multicloudj.common.exceptions.InvalidArgumentException if the requested
   *     status transition is invalid for the substrate
   * @throws UnsupportedOperationException if the provider does not support bucket versioning
   *     configuration
   */
