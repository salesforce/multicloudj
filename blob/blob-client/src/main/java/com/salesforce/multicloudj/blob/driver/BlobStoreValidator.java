...
public class BlobStoreValidator {
  ...
  static final String INVALID_OBJECT_NAME_COLLECTION_MSG =
      "Collection of object names cannot be null or empty";
  static final String INVALID_BLOB_IDENTIFIERS_COLLECTION_MSG =
      "Collection of blob identifiers cannot be null or empty";
  ...
  static final String DURATION_INVALID_MSG =
      "Duration must be a non-null non-zero positive value. Duration was '%s'";
  static final String PRESIGNED_URL_OPERATION_INVALID_MSG =
      "The presigned url operation type must be non-null";
  static final String INVALID_ENDPOINT_NULL_MSG = "Endpoint cannot be null";
  static final String INVALID_ENDPOINT_PROTOCOL_MSG =
      "Endpoint must have a protocol of http or https. value=%s";
  static final String INVALID_ENDPOINT_HOSTNAME_MSG =
      "Endpoint must have a non-empty hostname value. value=%s";
  static final String INVALID_ENDPOINT_PORT_MSG =
      "Endpoint must have a non-zero positive value for the port number. value=%s";
  static final String INVALID_ENDPOINT_NO_PATH_ALLOWED_MSG =
      "Endpoint must not have a path specified. value=%s";
  static final String INVALID_ENDPOINT_NO_QUERY_PARAMS_ALLOWED_MSG =
      "Endpoint must not have query params specified. value=%s";
  static final String INVALID_MAX_CONNECTIONS_MSG =
      "Maximum connections must be a positive value. value=%s";
  static final String INVALID_SOCKET_TIMEOUT_MSG =
      "Socket timeout must be a non-negative value if specified. Duration was '%s'";
  static final String INVALID_RANGED_READ_NEGATIVE_BOUNDARIES_MSG =
  ...
  /**
   * Inspects the input collection and throws an IllegalArgumentException if the collection is
   * `null`, empty, or contains any values that are null, empty, or blank.
   *
  ...
  /**
   * Inspects the strings and throws an IllegalArgumentException if they don't match. This is a
  ...
  /**
   * Validates that the supplied key is not empty. This is identical to calling {@code
  ...
  /**
   * Validates that the supplied bucket is not empty. This is identical to calling {@code
  ...
  /**
   * Validates that the supplied keys are not empty. This is identical to calling {@code
  ...
  /**
   * Validates that the supplied list of objects are not empty or null. Also validates that the keys
  ...
  /**
   * Validates the input request. This is identical to calling {@code validateKey(request.getKey())}
   *
  ...
  /**
   * Validates the input request to ensure it has valid {srcKey, destKey, destBucket}. This is
  ...
  /**
   * Validates the input request to ensure it has valid {srcBucket, srcKey, destKey}. This is
   * identical to calling
   *
   * <pre>{@code
  ...
  /**
   * Validates the input request. This validates that the request is not null and that the local
   * source directory is not null or empty.
   *
   * @param request the request to inspect.
   */
  public void validate(DirectoryUploadRequest request) {
    ...
    if (request.getLocalSourceDirectory() == null
        || request.getLocalSourceDirectory().trim().isEmpty()) {
  ...
  /**
   * Validates the input request. This validates that the request is not null and that the local
   * destination directory is not null or empty.
   *
   * @param request the request to inspect.
   */
  public void validate(DirectoryDownloadRequest request) {
    ...
    if (request.getLocalDestinationDirectory() == null
        || request.getLocalDestinationDirectory().trim().isEmpty()) {
  ...
  /**
   * Validates that a URI specifies the schema/protocol is http/https, and that the hostname is
   * non-empty and non-null, and optionally validates that the port exists and is a positive values.
   *
  ...
  public void validateEndpoint(URI endpoint, boolean requirePort) {
    ...
    boolean isProtocolValid =
        StringUtils.equalsIgnoreCase(protocol, "http")
            || StringUtils.equalsIgnoreCase(protocol, "https");
    if (!isProtocolValid) {
    ...
    if (StringUtils.isBlank(endpoint.getHost())) {
      throw new IllegalArgumentException(
          String.format(INVALID_ENDPOINT_HOSTNAME_MSG, endpoint.getHost()));
    }
    if (requirePort && endpoint.getPort() < 0) {
      throw new IllegalArgumentException(
          String.format(INVALID_ENDPOINT_PORT_MSG, endpoint.getPort()));
    }
    if (!StringUtils.isBlank(endpoint.getPath()) && !endpoint.getPath().equals("/")) {
      throw new IllegalArgumentException(
          String.format(INVALID_ENDPOINT_NO_PATH_ALLOWED_MSG, endpoint.getPath()));
    }
    if (endpoint.getQuery() != null) {
      throw new IllegalArgumentException(
          String.format(INVALID_ENDPOINT_NO_QUERY_PARAMS_ALLOWED_MSG, endpoint.getQuery()));
    }
  }
  ...
  public void validateMaxConnections(Integer maxConnections) {
    if (maxConnections == null || maxConnections <= 0) {
      throw new IllegalArgumentException(
          String.format(INVALID_MAX_CONNECTIONS_MSG, maxConnections));
    }
  }
  ...
  public void validateRange(Long start, Long end) {
    if ((start != null && start < 0) || (end != null && end < 0)) {
      throw new IllegalArgumentException(
          String.format(INVALID_RANGED_READ_NEGATIVE_BOUNDARIES_MSG, start, end));
    }
