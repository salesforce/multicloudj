...
public class ProviderSupplier {
  ...
  static AbstractBlobStore.Builder findProviderBuilder(String providerId) {
    ...
    Iterable<AbstractBlobStore> all =
        ImmutableSet.<AbstractBlobStore>builder().addAll(services).build();
  ...
  static AbstractBlobClient.Builder findBlobClientProviderBuilder(String providerId) {
    ...
    Iterable<AbstractBlobClient> all =
        ImmutableSet.<AbstractBlobClient>builder().addAll(services).build();
  ...
  private static AbstractBlobClient.Builder<?> createBlobClientBuilderInstance(
      AbstractBlobClient<?> provider) {
    try {
      return (AbstractBlobClient.Builder<?>)
          provider.getClass().getMethod("builder").invoke(provider);
    } catch (Exception e) {
      throw new RuntimeException(
          "Failed to create blob client builder for provider: " + provider.getClass().getName(), e);
    }
  }
}
