...
/**
 * An async wrapper around AbstractBlobStore that implements AsyncBlobStore. This wrapper takes an
 * AbstractBlobStore and makes it asynchronous by executing operations on a provided
 * ExecutorService.
 */
public class BlobStoreAsyncBridge implements AsyncBlobStore {

  @Getter private final AbstractBlobStore blobStore;

  @Getter private final ExecutorService executorService;

  /**
   * Creates a new async wrapper around the provided BlobStore. Note: This is only a stop-gap for
   * vendors that don't currently provide async client support. Once all the substrates have async
   * clients the plan is to deprecate and remove this class.
   *
   * @param blobStore the synchronous blob store to wrap
   * @param executorService the executor service to use for async operations. If this value is null
   *     then this will use the ForkJoinPool.commonPool()
   */
  ...
  public CompletableFuture<DownloadResponse> download(
    ...
    return CompletableFuture.supplyAsync(
        () -> blobStore.download(downloadRequest, outputStream), executorService);
  }
  ...
  public CompletableFuture<BlobMetadata> getMetadata(String key, String versionId) {
    return CompletableFuture.supplyAsync(
        () -> blobStore.getMetadata(key, versionId), executorService);
  }
  ...
  public CompletableFuture<MultipartUpload> initiateMultipartUpload(
    ...
    return CompletableFuture.supplyAsync(
        () -> blobStore.initiateMultipartUpload(request), executorService);
  }
  ...
  public CompletableFuture<UploadPartResponse> uploadMultipartPart(
    ...
    return CompletableFuture.supplyAsync(
        () -> blobStore.uploadMultipartPart(mpu, mpp), executorService);
  }
  ...
  public CompletableFuture<MultipartUploadResponse> completeMultipartUpload(
    ...
    return CompletableFuture.supplyAsync(
        () -> blobStore.completeMultipartUpload(mpu, parts), executorService);
  }
  ...
  public CompletableFuture<DirectoryDownloadResponse> downloadDirectory(
    ...
    return CompletableFuture.supplyAsync(
        () -> blobStore.downloadDirectory(directoryDownloadRequest), executorService);
  }
  ...
  public CompletableFuture<DirectoryUploadResponse> uploadDirectory(
    ...
    return CompletableFuture.supplyAsync(
        () -> blobStore.uploadDirectory(directoryUploadRequest), executorService);
  }
