...
/**
 * This test is meant to verify that the async ProviderSupplier can load each of the substrate
 * implementation. Extend this class in each of the substrate implementations.
 */
public abstract class AsyncProviderSupplierIT {
  ...
  void testAsyncProviderSupplier() {
    StsCredentials credentials =
        new StsCredentials("accessKeyId", "accessKeySecret", "securityToken");
    CredentialsOverrider credentialsOverrider =
        new CredentialsOverrider.Builder(CredentialsType.SESSION)
            .withSessionCredentials(credentials)
            .build();
    AsyncBucketClient bucketClient =
        AsyncBucketClient.builder(getSubstrate())
