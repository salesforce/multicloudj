...
public class GcpTransformer {
  ...
  /**
   * No range specified - computeRange(null, null) - (null, null) Reading the first 500 bytes -
   * computeRange(0, 500) - (0, 501) Reading a middle 500 bytes - computeRange(123, 623) - (123,
   * 624) Reading the last 500 bytes - computeRange(null, 500) - (size-500, size+1) Reading
   * everything but first 500 bytes - computeRange(500, null) - (500, null)
   */
  protected Pair<Long, Long> computeRange(Long start, Long end, long fileSize) {
    // Need to validate range here because read from blob store API fails silently without any
    // exception
    if (start != null && start > fileSize) {
      throw new IllegalArgumentException(
          "Start of range cannot be greater than file size: " + start);
    }
  ...
  public DownloadResponse toDownloadResponse(Blob blob) {
    return DownloadResponse.builder().key(blob.getName()).metadata(toBlobMetadata(blob)).build();
  }
  ...
  public Storage.CopyRequest toCopyRequest(CopyRequest request) {
    ...
    return Storage.CopyRequest.newBuilder().setSource(source).setTarget(target).build();
  }

  public Storage.CopyRequest toCopyRequest(CopyFromRequest request) {
    BlobId source =
        toBlobId(request.getSrcBucket(), request.getSrcKey(), request.getSrcVersionId());
      ...
      if (objectLock.getRetainUntilDate() != null) {
        ...
        builder.setRetention(
            Retention.newBuilder()
                ...
                .setRetainUntilTime(
                    OffsetDateTime.ofInstant(objectLock.getRetainUntilDate(), ZoneOffset.UTC))
                .build());
      }

      // Set object holds (legal hold)
      boolean useEventBased =
          objectLock.getUseEventBasedHold() != null ? objectLock.getUseEventBasedHold() : false;
  ...
  /**
   * Converts a DirectoryUploadRequest to a list of file paths to upload. This method handles
   * directory traversal and filtering based on the request parameters.
   *
   ...
   */
  public List<List<com.salesforce.multicloudj.blob.driver.BlobInfo>> partitionList(
      List<com.salesforce.multicloudj.blob.driver.BlobInfo> blobInfos, int partitionSize) {
  ...
  public RetrySettings toGcpRetrySettings(RetryConfig retryConfig) {
    ...
    if (retryConfig.getAttemptTimeout() != null) {
      ...
      settingsBuilder
          .setInitialRpcTimeoutDuration(attemptTimeout)
   ...
   */
  public List<BlobIdentifier> toBlobIdentifiers(
      List<com.salesforce.multicloudj.blob.driver.BlobInfo> blobList) {
