...
/**
 * An implementation of the {@link AbstractBlobClient} for GCP Cloud Storage. GcpBlobClient is
 * service client for interacting with GCP Cloud Blob Storage.
 *
...
public class GcpBlobClient extends AbstractBlobClient<GcpBlobClient> {
   ...
   *
   * @return a {@link ListBucketsResponse} containing a list of {@link BucketInfo} objects
   *     representing the buckets for this account.
   */
  @Override
          ...
          BucketInfo.builder()
              ...
              .creationDate(
                  Instant.ofEpochMilli(
                      bucket.getCreateTimeOffsetDateTime().toInstant().toEpochMilli()))
              .build();
      bucketInfoList.add(bucketInfo);
    }

    return ListBucketsResponse.builder().bucketInfoList(bucketInfoList).build();
  }
  ...
  private static Storage buildStorage(Builder builder) {
    ...
    if (builder.getProxyEndpoint() != null) {
      ...
      org.apache.http.impl.client.HttpClientBuilder httpClientBuilder =
          org.apache.http.impl.client.HttpClientBuilder.create().setProxy(proxy);
      storageBuilder.setTransportOptions(
