...
/**
 * Relax the request body matching for ack/nack/pull operations. Use JsonPath to only verify the
 * existence.
 */
public class AckMatcherRelaxingTransformer extends StubMappingTransformer {
  ...
  public StubMapping transform(StubMapping stub, FileSource files, Parameters params) {
    ...
    boolean isAck =
        Stream.of(url, urlPattern, urlPath)
    ...
    if (isAck || isMod) {
      ...
      // (pushEndpoint is a user configuration that appears in the request body, unlike service
      // endpoints)
      List<ContentPattern<?>> bodyPatterns = stub.getRequest().getBodyPatterns();
      if (bodyPatterns != null) {
        bodyPatterns.clear();
