...
public class GcpPubsubIT extends AbstractPubsubIT {
  ...
  protected Harness createHarness() {
    ...
    logger.info(
        "WireMock ports: HTTPS={}, HTTP={} (use HTTP port for ngrok if testing push)",
        harnessImpl.getPort(),
        harnessImpl.getWireMockHttpPort());
    return harnessImpl;
  }
  ...
  public static class HarnessImpl implements Harness {
    ...
    public AbstractTopic createTopicDriver() {
      ...
      try {
        TopicAdminSettings.Builder settingsBuilder =
    ...
    /**
     * Create a subscription with an index suffix. This allows creating multiple subscriptions to
     * the same topic.
     */
    @Override
    ...
    private AbstractSubscription buildSubscription(int index, Duration nackVisibilityTimeout) {
      ...
      try {
        ...
        String subscriptionNameWithIndex =
            index > 0 ? subscriptionName + "-" + index : subscriptionName;
        String fullSubscriptionName =
            "projects/" + GcpPubsubIT.PROJECT_ID + "/subscriptions/" + subscriptionNameWithIndex;
    ...
    /**
     * Creates a push subscription with the specified push endpoint. This method is used for testing
     * push delivery mode.
     */
    public void createPushSubscription(
        String subscriptionName, String topicName, String pushEndpoint) throws IOException {
      boolean isRecordingEnabled = System.getProperty("record") != null;
      TransportChannelProvider channelProvider = TestsUtilGcp.getTransportChannelProvider(port);
      ...
      String fullSubscriptionName =
          "projects/" + GcpPubsubIT.PROJECT_ID + "/subscriptions/" + subscriptionName;
      String fullTopicName = "projects/" + GcpPubsubIT.PROJECT_ID + "/topics/" + topicName;
    ...
    try (AbstractTopic topic = harnessImpl.createTopicDriver()) {
      ...
      Message testMessage =
          Message.builder()
              ...
              .withMetadata(
                  Map.of(
                      "test-type", "push", "timestamp", String.valueOf(System.currentTimeMillis())))
              .build();
