...
public class GcpTopic extends AbstractTopic<GcpTopic> {
  ...
  /**
   * Override batcher options to align with GCP Pub/Sub limits. GCP supports up to 1000 messages or
   * 10MB per batch.
   */
  @Override
  ...
  protected void doSendBatch(List<Message> messages) {
    ...
    List<PubsubMessage> pubsubMessages =
        messages.stream()
            .map(
                m -> {
                  PubsubMessage.Builder b =
                      PubsubMessage.newBuilder().setData(ByteString.copyFrom(m.getBody()));
