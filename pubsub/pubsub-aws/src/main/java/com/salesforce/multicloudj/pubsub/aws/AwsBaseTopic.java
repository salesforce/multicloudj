 ...
 *
 * <p>This class contains shared logic for metadata encoding, base64 encoding, and error handling
 * that is common to both SQS and SNS implementations.
 */
public abstract class AwsBaseTopic<T extends AwsBaseTopic<T>> extends AbstractTopic<T> {
  ...
  /**
   * Encodes message body based on topic options and returns the encoded body along with a flag
   * indicating whether base64 encoding was applied.
   */
   ...
   *
   * <p>AWS SQS/SNS message attribute names can only contain alphanumeric characters, underscores
   * (_), hyphens (-), and periods (.). Special characters like spaces, slashes, etc. are not
   * allowed. Additionally, periods cannot be at the start of the key or follow another period.
   *
   ...
   *
   * <p>Examples: - "file/path" → "file__0x2F__path" (0x2F = '/') - "user name" → "user__0x20__name"
   * (0x20 = ' ') - "type:message" → "type__0x3A__message" (0x3A = ':') - ".key" → "__0x2E__key"
   * (period at start) - "key..name" → "key__0x2E____0x2E__name" (consecutive periods)
   */
  ...
  public enum Base64EncodingStrategy {
    /**
     * Automatically determines if encoding is required. Valid UTF-8 text is sent as-is; invalid
     * sequences are base64 encoded, and a MessageAttribute with key "base64encoded" is added to the
     * message.
     */
    AUTO,
    /**
     * Always means that all message bodies are base64 encoded. A MessageAttribute with key
     * "base64encoded" is added to the message.
     */
    ALWAYS,
    /**
     * Never means that message bodies are never base64 encoded. Non-UTF-8 bytes in message bodies
     * may be modified by SNS/SQS.
     */
    NEVER
  }
  ...
  public abstract static class Builder<
          TBuilder extends Builder<TBuilder, TTopic>, TTopic extends AwsBaseTopic<TTopic>>
