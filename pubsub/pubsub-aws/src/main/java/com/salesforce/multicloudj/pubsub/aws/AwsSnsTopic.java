...
/** AWS SNS implementation of AbstractTopic. Handles sending messages to SNS topics. */
...
public class AwsSnsTopic extends AwsBaseTopic<AwsSnsTopic> {
  ...
  private void sendToSns(List<Message> messages) {
    ...
    for (int i = 0; i < messages.size(); i++) {
      ...
      PublishBatchRequestEntry.Builder entryBuilder =
    ...
    if (!batchResponse.failed().isEmpty()) {
      ...
      throw new SubstrateSdkException(
          String.format(
              "SNS PublishBatch failed for %d message(s): %s, %s",
  ...
  private void reviseSnsEntryAttributes(
      Message message, PublishBatchRequestEntry.Builder entryBuilder) {
  ...
  private Map<String, MessageAttributeValue> convertMetadataToSnsAttributes(
  ...
  public static class Builder extends AwsBaseTopic.Builder<Builder, AwsSnsTopic> {
    ...
    /**
     * Validates that the provided topic ARN points to an existing topic. Throws an exception if the
     * topic does not exist or is inaccessible.
     */
