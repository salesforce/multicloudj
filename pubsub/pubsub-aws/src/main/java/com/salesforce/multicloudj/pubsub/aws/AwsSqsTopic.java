...
/** AWS SQS implementation of AbstractTopic. Handles sending messages to SQS queues. */
...
public class AwsSqsTopic extends AwsBaseTopic<AwsSqsTopic> {
  ...
  private void sendToSqs(List<Message> messages) {
    ...
    for (int i = 0; i < messages.size(); i++) {
      ...
      if (encodingResult.isBase64Encoded()) {
        ...
        attributes.put(
            MetadataKeys.BASE64_ENCODED,
            MessageAttributeValue.builder().dataType("String").stringValue("true").build());
      }
    ...
    if (!batchResponse.failed().isEmpty()) {
      ...
      throw new SubstrateSdkException(
          String.format(
              "SQS SendMessageBatch failed for %d message(s): %s, %s",
              batchResponse.failed().size(), firstFailure.code(), firstFailure.message()));
    }
  }

  /** Sets SQS-specific attributes on a SendMessageBatchRequestEntry based on message metadata. */
  private void reviseSqsEntryAttributes(
      Message message, SendMessageBatchRequestEntry.Builder entryBuilder) {
  ...
  private Map<String, MessageAttributeValue> convertMetadataToSqsAttributes(
    ...
    if (metadata != null && !metadata.isEmpty()) {
      for (Map.Entry<String, String> entry : metadata.entrySet()) {
        ...
        if (MetadataKeys.DEDUPLICATION_ID.equals(key)
        ...
        attributes.put(
            encodedKey,
            MessageAttributeValue.builder().dataType("String").stringValue(encodedValue).build());
      }
    }
    return attributes;
  }
  ...
  public static class Builder extends AwsBaseTopic.Builder<Builder, AwsSqsTopic> {
    ...
    /**
     * Directly set the topic URL to avoid calling GetQueueUrl again. Used when the queue URL has
    ...
    /**
     * Validates that the provided queue URL points to an existing queue. Throws an exception if the
     * queue does not exist or is inaccessible.
     */
    private static void validateQueueUrlExists(String queueUrl, SqsClient sqsClient) {
      ...
      // All exceptions will be handled by getException() which maps them to appropriate
      // SubstrateSdkException types
