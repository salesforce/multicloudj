 ...
 *
 * <p>This is needed because when WireMock proxies requests to AWS services, the original signature
 * becomes invalid. This transformer re-signs the request using the actual AWS credentials before
 * forwarding to the real AWS service.
 *
 * <p>Supports both SNS and SQS services by detecting the service from the hostname.
 */
public class PubsubReplaceAuthHeaderTransformer implements StubRequestFilterV2 {

  private static final Pattern SNS_REGION_PATTERN =
      Pattern.compile("sns\\.(.*?)\\.amazonaws\\.com");
  ...
  private String computeAuthHeader(Request request) throws URISyntaxException {
    ...
    if (request.containsHeader("X-Amz-Security-Token")) {
      requestToSign.putHeader(
          "X-Amz-Security-Token", request.header("X-Amz-Security-Token").values());
    }
    ...
    final SignedRequest signerOutput =
        signer.sign(
            r ->
                r.identity(DefaultCredentialsProvider.create().resolveCredentials())
