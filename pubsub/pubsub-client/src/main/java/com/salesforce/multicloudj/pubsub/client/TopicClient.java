 ...
 *
 * <p>TopicClient provides a simplified interface for sending messages to cloud pubsub queue/topic
 * in a provider-agnostic way. It handles batching, retries, and error conversion automatically.
 */
public class TopicClient implements AutoCloseable {
   ...
   *
   * <p>This method will block until the message is successfully sent or an error occurs. Messages
   * are automatically batched by the underlying driver for efficiency.
   *
   ...
   *
   * <p>Shuts down the topic client and releases all resources. After calling this method, no more
   * messages can be sent through this client. Any pending messages will be flushed before shutdown
   * completes. It's safe to call this method multiple times.
   *
