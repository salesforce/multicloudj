 ...
 *
 * <p>ProviderSupplier uses Java's ServiceLoader mechanism to automatically discover topic and
 * subscription provider implementations at runtime. This enables the multicloud SDK's plugin
 * architecture where new cloud providers can be added without modifying existing code.
 *
 * <p>Provider implementations register themselves using the {@code @AutoService} annotation and are
 * automatically discovered when their JARs are on the classpath.
 */
public class ProviderSupplier {
  ...
  static AbstractSubscription.Builder<?> findSubscriptionProviderBuilder(String providerId) {
    ...
    throw new IllegalArgumentException(
        "No Subscription provider found for providerId: " + providerId);
  }
  ...
  private static AbstractTopic.Builder<?> createTopicBuilderInstance(AbstractTopic<?> provider) {
    try {
      ...
      throw new RuntimeException(
          "Failed to create topic builder for provider: " + provider.getClass().getName(), e);
    }
  }
   ...
   * @throws RuntimeException if the builder creation fails
   */
  private static AbstractSubscription.Builder<?> createSubscriptionBuilderInstance(
      AbstractSubscription<?> provider) {
    try {
      return (AbstractSubscription.Builder<?>)
          provider.getClass().getMethod("builder").invoke(provider);
    } catch (Exception e) {
      throw new RuntimeException(
          "Failed to create subscription builder for provider: " + provider.getClass().getName(),
          e);
    }
  }
}
