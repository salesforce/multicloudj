 ...
 *
 * <p>All message sending operations use the Batcher infrastructure.
 *
 * <p>Features provided: - Batching using batcher infrastructure and default batcher configuration -
 * Message validation and preprocessing - Exception propagation (cloud providers handle their own
 * exception mapping) - Before/after send hooks for extensibility - Implementation of send and
 * sendBatch methods
 *
 * <p>Drivers need to provide: - doSendBatch(List) - Provider-specific synchronous batch sending -
 * Override batching configuration methods for provider-specific settings - Override hook methods
 * for custom pre/post-send behavior
 */
public abstract class AbstractTopic<T extends AbstractTopic<T>> implements AutoCloseable, Provider {
  ...
  /**
   * Creates batcher options for this topic. Provides defaults that cloud providers can override to
   * provide provider-specific batching configuration that aligns with their service limits and
  ...
  /**
   * Sends a single message synchronously with error handling. This method blocks until the message
  ...
  /**
   * Sends a batch of messages synchronously with full validation and error handling. This method
   * blocks until all messages are successfully sent or an error occurs.
   */
  private final void sendBatch(List<Message> messages) {
    ...
    try {
      ...
      throw new SubstrateSdkException(
          "Interrupted while waiting for pubsub batch send to complete", e);
    }
  }
   ...
   *
   * <p>This method should: - Flush any pending outbound messages - Close network connections - Stop
   * background threads - Release any other resources
   *
   * <p>After calling this method, the topic should not accept new messages. It's safe to call this
