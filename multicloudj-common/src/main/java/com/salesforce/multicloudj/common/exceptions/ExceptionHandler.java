...
public class ExceptionHandler {
  ...
  /**
   * Handles the given exception t which are the provider specific exceptions, wraps and propagates
   * it in a specific SubstrateSdkException. This serves as an abstraction layer for the exception
   * handling. So our end users can get to deal with substrate agnostic exceptions and also can get
   * the provider specific exceptions if they are interested in going for detail.
   *
   * <p>This method examines the provided exception class and throws a corresponding
   * SubstrateSdkException based on the type. If no specific match is found, it throws an
   * UnknownException.
   *
   * @param exceptionClass The class of the exception to be handled, this is SDK defined exception
   *     for abstraction.
