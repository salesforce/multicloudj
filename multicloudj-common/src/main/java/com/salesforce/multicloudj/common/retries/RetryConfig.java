...
public final class RetryConfig {
  ...
  public enum Mode {
    /**
     * Exponential backoff mode where delay grows exponentially between retries. Uses formula:
     * {@code delay = min(maxDelayMillis, initialDelayMillis * multiplier^(attempt-1))}
     *
  ...
  /**
   * Maximum number of attempts including the initial request. For example, {@code maxAttempts = 3}
   * means 1 initial attempt + 2 retries. If null, the cloud provider's default will be used.
   */
  private final Integer maxAttempts;

  /**
   * Initial delay in milliseconds before the first retry (EXPONENTIAL mode only). This is the base
   * delay that gets multiplied by {@code multiplier^(attempt-1)}.
   */
  ...
  /**
   * Maximum delay cap in milliseconds (EXPONENTIAL mode only). Prevents delay from growing
   * indefinitely by capping it at this value.
   */
  private final long maxDelayMillis;

  /**
   * Fixed delay in milliseconds between retries (FIXED mode only). The same delay is used for all
   * retry attempts.
   */
   ...
   * If set, each individual request attempt will be subject to this timeout. If null, the cloud
   * provider's default attempt timeout will be used.
   */
  private final Long attemptTimeout;

  /**
   * Optional total timeout in milliseconds for all retry attempts combined. If set, the retry logic
   * will stop retrying once this timeout is exceeded, even if {@code maxAttempts} has not been
