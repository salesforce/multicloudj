 ...
 * <p>Usage example:
 * <pre>{@code
 * DBBackRestoreClient client = DBBackRestoreClient.builder("aws")
 *     .withRegion("us-west-2")
 *     .withCollectionName("my-table")
 *     .build();
 *
 * // List backups
 ...
 *     .backupId("backup-123")
 *     .targetCollectionName("my-restored-table")
 *     .build();
...
public class DBBackRestoreClient implements AutoCloseable {
   ...
   */
  public static ClientBuilder builder(String providerId) {
    return new ClientBuilder(providerId);
  }
   ...
   *
   * <p>For AWS DBBackRestore, the {@code request.options} map should contain the IAM role ARN with
  ...
  public void deleteBackup(String backupId) {
    dbBackRestore.deleteBackup(backupId);
  }
  ...
  public String getRegion() {
    return dbBackRestore.getRegion();
  }

  /**
   * Gets the collection/table name for this client.
   *
   * @return the collection/table name
   */
  public String getCollectionName() {
    return dbBackRestore.getCollectionName();
  }
  ...
  public static class ClientBuilder {
    ...
    private AbstractDBBackRestore.Builder<?, ?> dbBackRestoreBuilder;
    private AbstractDBBackRestore dbBackRestore;
     ...
     * @param providerId the cloud provider identifier (e.g., "aws", "gcp-firestore", "ali")
     */
    ClientBuilder(String providerId) {
    ...
    /**
     * Sets the collection/table name for backup/restore operations.
     *
     * @param collectionName the name of the collection/table
     * @return this builder
     */
    public ClientBuilder withCollectionName(String collectionName) {
      this.dbBackRestoreBuilder.withCollectionName(collectionName);
      return this;
    }
    ...
    public DBBackRestoreClient build() {
      this.dbBackRestore = this.dbBackRestoreBuilder.build();
      return new DBBackRestoreClient(this.dbBackRestore);
    }
  }
}
