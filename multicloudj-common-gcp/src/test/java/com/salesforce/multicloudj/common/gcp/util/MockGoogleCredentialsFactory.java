...
/**
 * Factory for creating mock {@link GoogleCredentials} instances for playback-mode tests. The
 * credentials override {@code refreshAccessToken()} to return a static long-lived token, preventing
 * external calls during test playback.
 */
public final class MockGoogleCredentialsFactory {
  ...
  private static class MockGoogleCredentialsWithIdToken extends GoogleCredentials
    ...
    public IdToken idTokenWithAudience(
        String targetAudience, java.util.List<IdTokenProvider.Option> options) throws IOException {
      // Return a mock ID token for testing
      // Create a JWT with the requested audience
      // CHECKSTYLE.SUPPRESS: LineLength for +5 lines
      String mockJwt =
