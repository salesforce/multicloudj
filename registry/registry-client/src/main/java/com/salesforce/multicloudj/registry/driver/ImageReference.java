 ...
 *
 * <p>User passes one string (e.g. "my-image:latest" or "my-image@sha256:abc..."); we split it
 * because the API uses separate path segments: GET /v2/{repository}/manifests/{reference} and GET
 * /v2/{repository}/blobs/{digest}.
 *
 * <p>
 *
 * <ul>
 *   <li>Split on "@" delimiter
 *   <li>Validate digest algorithm prefix (must be "sha256:")
 *   <li>Validate digest hex format: 64 lowercase hex characters [a-f0-9]{64}
 * </ul>
 */
public final class ImageReference {
  ...
  private static final Pattern SHA256_HEX_PATTERN =
      Pattern.compile("^[a-f0-9]{" + SHA256_HEX_LENGTH + "}$");
  ...
  private static void validateSha256Hex(String hex, String originalRef) {
    if (hex.length() != SHA256_HEX_LENGTH) {
      throw new InvalidArgumentException(
          String.format(
              "invalid checksum digest length: expected %d characters, got %d: %s",
    ...
    if (!SHA256_HEX_PATTERN.matcher(hex).matches()) {
      throw new InvalidArgumentException(
          String.format(
              "invalid checksum digest format: must be 64 lowercase hex characters [a-f0-9]: %s",
              originalRef));
    }
  }
