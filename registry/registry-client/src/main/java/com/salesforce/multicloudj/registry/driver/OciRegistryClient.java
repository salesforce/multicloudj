...
public class OciRegistryClient implements AutoCloseable {
    ...
    public Manifest fetchManifest(String repository, String reference) throws IOException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(request)) {
            ...
            org.apache.http.Header header = response.getFirstHeader("Docker-Content-Digest");
            ...
            if (digestHeader == null) {
                try {
                    ...
                    digestHeader = DIGEST_PREFIX + bytesToHex(hash);
    ...
    private Manifest parseManifestResponse(String responseBody, String digest) throws IOException {
        ...
        String mediaType = json.has("mediaType") ? json.get("mediaType").getAsString() : "";

        // Check if this is an image index (multi-arch)
        if (mediaType.contains("index") || mediaType.contains("manifest.list")
                || json.has("manifests")) {
    ...
    private Map<String, String> parseAnnotations(JsonObject json) {
        ...
        JsonObject annotationsObj = json.getAsJsonObject("annotations");
    ...
    private String parseSubject(JsonObject json) {
        ...
        JsonObject subjectObj = json.getAsJsonObject("subject");
        return subjectObj.has("digest") ? subjectObj.get("digest").getAsString() : null;
    }
    ...
    private Platform parsePlatform(JsonObject manifestDesc) {
        ...
        String operatingSystem = platformObj.has("os") ? platformObj.get("os").getAsString() : null;
        String architecture = platformObj.has("architecture") ? platformObj.get("architecture").getAsString() : null;
        String osVersion = platformObj.has("os.version") ? platformObj.get("os.version").getAsString() : null;
        String variant = platformObj.has("variant") ? platformObj.get("variant").getAsString() : null;
    ...
    private List<String> parseOsFeatures(JsonObject platformObj) {
        ...
        JsonArray osFeaturesArray = platformObj.getAsJsonArray("os.features");
    ...
    private Manifest parseImageIndex(JsonObject json, String digest) throws IOException {
        JsonArray manifestsArray = json.getAsJsonArray("manifests");
        ...
        for (JsonElement element : manifestsArray) {
            ...
            String manifestDigest = manifestDesc.get("digest").getAsString();
    ...
    private Manifest parseImageManifest(JsonObject json, String digest) throws IOException {
        ...
        if (json.has("config")) {
            ...
            if (config != null && config.has("digest")) {
                configDigest = config.get("digest").getAsString();
            }
        }
    ...
    private List<Manifest.LayerInfo> parseLayerInfos(JsonObject json) throws IOException {
        JsonArray layers = json.has("layers") ? json.getAsJsonArray("layers") : null;
        ...
        if (layers != null) {
            for (JsonElement element : layers) {
                ...
                String layerDigest = layer.get("digest").getAsString();
                String mediaType = layer.has("mediaType") ? layer.get("mediaType").getAsString() : null;
                Long size = layer.has("size") ? layer.get("size").getAsLong() : null;
