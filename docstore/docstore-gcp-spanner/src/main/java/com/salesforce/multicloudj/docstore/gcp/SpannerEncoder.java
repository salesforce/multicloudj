...
/**
 * SpannerEncoder implements the Encoder interface for Google Cloud Spanner. It converts Java
 * objects into Spanner Value types for storage in Spanner.
 *
 * <p>This encoder handles: - Primitive types (boolean, int, long, double, string) - Byte arrays -
 * Lists and arrays - Maps and structs - Null values
 */
public class SpannerEncoder implements Encoder {
  /** -- GETTER -- Gets the map of encoded Spanner Values. */
  @Getter private final Map<String, Value> map = new HashMap<>();

  private Value value;
  private List<Value> array;

  /**
   * Encodes a null value into a Spanner Value. The type of null value is determined by the context.
   */
  @Override
  ...
  /**
   * Throws an exception as this method should not be called directly. Use MapEncoder.mapKey()
   * instead.
   *
  ...
  /**
   * Creates a new encoder for encoding an array. This is equivalent to encodeList for Spanner.
   *
  ...
  /**
   * Gets the encoded Spanner Value. If the value is a map or array, it will be reconstructed from
   * the map or array field.
   *
        ...
        switch (firstElement.getType().getCode()) {
          ...
          default:
            throw new IllegalArgumentException(
                "Unsupported array element type: " + firstElement.getType());
        }
      }
    }
    return this.value;
  ...
  /**
   * MapEncoder is a specialized encoder for encoding map entries. It extends SpannerEncoder to
  ...
  /**
   * ListEncoder is a specialized encoder for encoding list elements. It extends SpannerEncoder to
