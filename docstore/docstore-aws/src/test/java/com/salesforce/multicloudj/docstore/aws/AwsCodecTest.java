...
public class AwsCodecTest {
  ...
  public void testEncodeValue() {
    ...
    List<TestCase> tests =
        Arrays.asList(
            ...
            // av().toBuilder().l(Collections.emptyList()).build()),
            //                new TestCase(Arrays.asList(1, 2), avl(avn("1"), avn("2"))),
            //                new TestCase(Arrays.asList(null, false), avl(nullValue,
            // AttributeValue.builder().bool(false).build())),
            new TestCase(
                new HashMap<String, Integer>(), av().toBuilder().m(new HashMap<>()).build()),
            new TestCase(
                Map.of("a", 1, "b", 2),
  ...
  public void testDecodeUnsupported() {
    ...
    List<TestCase> tests =
        Arrays.asList(
            new TestCase(
                AttributeValue.builder().ss("foo", "bar").build(), new ArrayList<String>()),
            new TestCase(
                AttributeValue.builder().ns("1.1", "-2.2", "3.3").build(), new ArrayList<Double>()),
            new TestCase(
                AttributeValue.builder()
                    .bs(
                        SdkBytes.fromByteBuffer(ByteBuffer.wrap(new byte[] {4})),
                        SdkBytes.fromByteBuffer(ByteBuffer.wrap(new byte[] {5})),
                        SdkBytes.fromByteBuffer(ByteBuffer.wrap(new byte[] {6})))
                    .build(),
                new ArrayList<byte[]>()));
    ...
    for (TestCase tc : tests) {
      ...
      decode(
          ...
          tc.out,
          d);
      // Curently we don't have decodelist. It should fail as unsupported here when we have it.
