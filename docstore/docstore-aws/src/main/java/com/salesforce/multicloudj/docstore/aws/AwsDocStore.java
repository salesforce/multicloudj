...
public class AwsDocStore extends AbstractDocStore {
  ...
  protected TableDescription getTableDescription() {
    if (tableDescription == null) {
      DescribeTableRequest describeTableRequest =
          DescribeTableRequest.builder().tableName(collectionOptions.getTableName()).build();
  ...
  public void runActions(List<Action> actions, Consumer<Predicate<Object>> beforeDo) {
    ...
    try {
      ...
      CompletableFuture<Void> writesTask =
          CompletableFuture.runAsync(() -> runWrites(writeList, beforeDo));
      CompletableFuture<Void> atomicWritesTask =
          CompletableFuture.runAsync(() -> runTxWrites(atomicWriteList, beforeDo));
  ...
  protected void batchGet(
    ...
    // AWS BatchGetItem expects a List<Map<String, AttributeValue>> where each Map represents one
    // document's key
    List<Map<String, AttributeValue>> keysList = new ArrayList<>();
    for (int i = start; i <= end; i++) {
      AttributeValue av =
          AwsCodec.encodeDocKeyFields(
              gets.get(i).getDocument(),
    ...
    if (responses.containsKey(collectionOptions.getTableName())) {
      ...
      for (Map<String, AttributeValue> item : items) {
        ...
        // There is a potential issue with the current docstore implementation if there are dup key
        // objects.
  ...
  private void runTxWrites(List<Action> writes, Consumer<Predicate<Object>> beforeDo) {
    ...
    TransactWriteItemsRequest request =
        TransactWriteItemsRequest.builder()
            .transactItems(txWrites)
            .clientRequestToken(UUID.uniqueString())
            .build();
  ...
  protected WriteOperation newPut(Action action, Consumer<Predicate<Object>> beforeDo) {
    ...
    if (mf.equals(collectionOptions.getPartitionKey())) {
      ...
      m.put(
          collectionOptions.getPartitionKey(), AttributeValue.builder().s(newPartitionKey).build());
    ...
    String precondition =
        buildPrecondition(action, expressionAttributeNames, expressionAttributeValues);
  ...
  protected void runPut(Put put, Action action, Consumer<Predicate<Object>> beforeDo) {
    PutItemRequest.Builder requestBuilder =
        PutItemRequest.builder().tableName(put.tableName()).item(put.item());
  ...
  protected String buildPrecondition(
      Action action,
    ...
    switch (action.getKind()) {
      ...
      case ACTION_KIND_DELETE:
        ...
        return buildRevisionPrecondition(
            action.getDocument(),
            getRevisionField(),
            expressionAttributeNames,
            expressionAttributeValues);
  ...
  private String buildRevisionPrecondition(
      Document doc,
      String revField,
      Map<String, String> expressionAttributeNames,
      Map<String, AttributeValue> expressionAttributeValues) {
  ...
  protected WriteOperation newDelete(Action action, Consumer<Predicate<Object>> beforeDo) {
    ...
    Delete delete =
        Delete.builder().tableName(collectionOptions.getTableName()).key(av.m()).build();
  ...
  protected void runDelete(Delete delete, Action action, Consumer<Predicate<Object>> beforeDo) {
    DeleteItemRequest.Builder requestBuilder =
        DeleteItemRequest.builder().tableName(delete.tableName()).key(delete.key());
    ...
    if (delete.expressionAttributeValues() != null
        && !delete.expressionAttributeValues().isEmpty()) {
  ...
  public DocumentIterator runGetQuery(Query query) {
    ...
    AwsDocumentIterator iter = new AwsDocumentIterator(qr, query, 0);

    iter.run(
        query.getPaginationToken() != null
            ? ((AwsPaginationToken) query.getPaginationToken()).getExclusiveStartKey()
            : null);
    return iter;
  }

  protected void checkPlan(QueryRunner qr) {
    if (qr.getScanRequest() != null
        && qr.getScanRequest().filterExpression() != null
        && !collectionOptions.isAllowScans()) {
  ...
  protected QueryRunner planQuery(Query query) {
    ...
    // Collect keys required for pagination. If table is used, the list contains primary keys of
    // base
    // table. If index is used, the list contains both base table primary keys and index primary
    // keys.
    ...
    if (queryable.indexName == null && queryable.key == null) {
      ...
      if (query.getFilters() != null && !query.getFilters().isEmpty()) {
        String filtersExpression =
            filtersToCondition(
                query.getFilters(), expressionAttributeNames, expressionAttributeValues);
    ...
    for (Filter filter : query.getFilters()) {
      String condition =
          toKeyCondition(
              ...
              expressionAttributeNames,
              expressionAttributeValues,
              queryable.getKey().getPartitionKey(),
              queryable.getKey().getSortKey());
      if (condition != null) {
    ...
    if (!filtersNoKey.isEmpty()) {
      String filtersExpression =
          filtersToCondition(filtersNoKey, expressionAttributeNames, expressionAttributeValues);
  ...
  protected Queryable getBestQueryable(Query query) {
    // If the query has an "=" filter on the table's partition key, look at the table and local
    // index.
    if (hasEqualityFilter(query, collectionOptions.getPartitionKey())) {
      ...
      if (hasFilter(query, collectionOptions.getSortKey())
          && orderingConsistent(query, collectionOptions.getSortKey())) {
      ...
      if (getTableDescription().localSecondaryIndexes() != null) {
        for (LocalSecondaryIndexDescription li : getTableDescription().localSecondaryIndexes()) {
          ...
          if (hasFilter(query, key.getSortKey())
              && localFieldIncluded(query, li)
              && orderingConsistent(query, key.getSortKey())) {
    ...
    if (getTableDescription().globalSecondaryIndexes() != null) {
      for (GlobalSecondaryIndexDescription gi : getTableDescription().globalSecondaryIndexes()) {
        ...
        if (hasEqualityFilter(query, key.partitionKey)
            && hasFilter(query, key.sortKey)
            && globalFieldIncluded(query, gi)
            && orderingConsistent(query, key.sortKey)) {
          return new Queryable(gi.indexName(), key);
        }
      }
    }

    // There are no matches for both partition and sort key. Now consider matches on partition key
    // only.
  ...
  private boolean localFieldIncluded(Query query, LocalSecondaryIndexDescription li) {
    return !query.getFieldPaths().isEmpty()
        || li.projection().projectionType() == ProjectionType.ALL;
  }
  ...
  private boolean orderingConsistent(Query query, String sortedField) {
    return query.getOrderByField() == null
        || query.getOrderByField().isEmpty()
  ...
  protected String toKeyCondition(
      Filter filter,
      Map<String, String> expressionAttributeNames,
      Map<String, AttributeValue> expressionAttributeValues,
      String pkey,
  ...
  protected String filtersToCondition(
      List<Filter> filters,
      Map<String, String> expressionAttributeNames,
      Map<String, AttributeValue> expressionAttributeValues) {
    if (filters.isEmpty()) {
    ...
    for (Filter filter : filters) {
      String filterExpression =
          filterToCondition(filter, expressionAttributeNames, expressionAttributeValues);
  ...
  private String filterToCondition(
      Filter filter,
      Map<String, String> expressionAttributeNames,
      Map<String, AttributeValue> expressionAttributeValues) {
    expressionAttributeNames.put("#attr" + filter.getFieldPath(), filter.getFieldPath());
    // Expression attribute name is fine to override, but expression attribute value shouldn't
    // override.
    String valueKey = ":value" + expressionAttributeValues.size();
    expressionAttributeValues.put(valueKey, encodeValue(filter.getValue()));
    switch (filter.getOp()) {
      ...
      case GREATER_THAN_OR_EQUAL_TO:
        return " #attr" + filter.getFieldPath() + " >= " + valueKey;
      case IN:
        return toInOrNotInCondition(
            filter, expressionAttributeNames, expressionAttributeValues, true);
      case NOT_IN:
        return toInOrNotInCondition(
            filter, expressionAttributeNames, expressionAttributeValues, false);
      default:
        throw new IllegalArgumentException("Invalid filter operation: " + filter.getOp());
    }
  }

  // The expressions for IN are of the format: #attrPlayer IN (:value0, :value1) where :value_i is
  // AttributeValue
  // defined in expressionAttributeValues map.
  // The expressions for NOT IN are of format same as IN but wrapped in NOT, for example: NOT
  // (#attrPlayer IN (:value0, :value1))
  private String toInOrNotInCondition(
      Filter filter,
      Map<String, String> expressionAttributeNames,
      Map<String, AttributeValue> expressionAttributeValues,
      boolean in) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Filter value is not collection type: " + filter.getFieldPath());
    }
  }
  ...
  public static class Builder extends AbstractDocStore.Builder<AwsDocStore, Builder> {
    ...
    private static DynamoDbClient buildDDBClient(Builder builder) {
      DynamoDbClientBuilder clientBuilder =
          DynamoDbClient.builder().region(Region.of(builder.getRegion()));
