...
public class AwsDocumentIterator implements DocumentIterator {
  ...
  // pagination token. If a return token is empty, it means there is no more element for the
  // request. This value
  // may be non-null even if this iterator has reached to the limit. The token then can be used by
  // the next query.
  ...
  private Map<String, AttributeValue> getKeysFromDocument(
      Document document, List<String> paginationKeys) {
  ...
  public void next(Document document) {
    ...
    paginationToken.setExclusiveStartKey(
        getKeysFromDocument(document, queryRunner.getPaginationKeys()));
