 ...
 * cloud providers.
 *
 * <p>This client supports various document operations such as create, read, update, delete (CRUD)
 * operations along with Query interface on documents stored in cloud-based document databases like
 * AWS DynamoDB, GCP Firestore, and Alibaba TableStore. The implementations also supports secondary
 * indexes behind the scenes and uses them to optimize the queries.
 *
 * <p>Usage example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The client supports input and output data in the form of Java POJOs and maps. The standard
 * types supported are String, Number, Boolean, Date, List and Map. List and Maps are not supported
...
public class DocStoreClient {
  ...
  /**
   * Protected constructor for DocStoreClient. Use the builder pattern to create instances.
   *
  ...
  public static class DocStoreClientBuilder {
    ...
    /**
     * Sets the instance ID for the document store. This is typically used for cloud providers that
     * require specific instance identification.
     *
     ...
     * @return An instance of self
     */
    public DocStoreClientBuilder withCredentialsOverrider(
  ...
  /**
   * Creates a new document in the document store. This operation will fail if a document with the
  ...
  /**
   * Replaces an existing document in the document store. This operation will fail if the document
   * does not exist.
   *
  ...
  /**
   * Creates or updates a document in the document store. This operation will create the document if
   * it doesn't exist, or update it if it does.
   *
  ...
  /**
   * Deletes a document from the document store. This operation is idempotent - it will not fail if
  ...
  /**
   * Retrieves a document from the document store. The document input should have a valid key. The
   * retrieved data is stored in the supplied document.
   *
  ...
  /**
   * Retrieves multiple documents from the document store in a single batch operation. This is more
   * efficient than multiple individual get operations. The retrieved data is stored in the supplied
   * documents.
   *
   * @param documents the list of documents to retrieve, each must have a valid key. Results will be
  ...
  /**
   * Creates or updates multiple documents in the document store in a single batch operation. This
   * is more efficient than multiple individual put operations. Each document will be created if it
   * doesn't exist, or updated if it does.
   *
  ...
  /**
   * Closes the document store client and releases associated resources. After calling this method,
  ...
  /**
   * Returns the underlying ActionList for advanced batch operations. This allows for fine-grained
   * control over batch operations and mixing different operation types.
   *
  ...
  /**
   * Creates a new Query instance for querying the document store. This allows for complex queries
