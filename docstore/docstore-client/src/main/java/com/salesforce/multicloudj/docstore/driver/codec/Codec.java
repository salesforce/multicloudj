...
/**
 * Codec providers the utility for encoding and decoding the data between java native types to the
 * target types such as substrate specific types.
 */
public class Codec {
  /**
   ...
   * @param enc the encoder used to encode the value. Every Encoder is supposed to hold the encoded
   *     value in their own way. For example: ListEncoder can have the List/Arrays and the
  ...
  /**
   * Encodes the fields of the provided object of the user defined class using an encoder. This
   * method uses reflection to access the object's fields, encode their values, and write them to
   * the {@code Encoder} in a map format.
   *
   * <p>The method retrieves the field information from the {@code FieldCache}, iterates through the
   * fields, and encodes each field value. If the field is a custom class, it recursively encodes
   * the nested object. Primitive or simple types are encoded directly. The field names are used as
   * the map keys.
   *
   * @param obj the object whose fields will be encoded. This object must not be {@code null}.
   * @param cache the {@code FieldCache} containing cached field information for the object's class.
   *     This cache provides the metadata needed to encode the fields. Must not be {@code null}.
   * @param enc the {@code Encoder} responsible for encoding the object into a map. The encoder
   *     creates key-value pairs where field names are used as keys, and the encoded field values
   *     are used as the values. Must not be {@code null}.
   * @throws NullPointerException if any of the arguments are {@code null}.
   * @throws RuntimeException if reflection fails during field access or if an object field is not
   *     accessible.
   */
  ...
  /**
   * Decodes the value in the decoder which is expected to have Map, updating the fields of the
   * Object obj with the decoded values. This method uses reflection to retrieve and decode fields
   * dynamically based on the metadata cached in the {@code FieldCache} and the map of key-value
   * pairs decoded by the {@code Decoder}.
   *
   * <p>For each field, the decoder processes a key-value pair, finds the corresponding field by its
   * name, and decodes the value. If the field is a custom class, it recursively decodes its fields.
   * Primitive or non-user defined types are decoded directly.
   *
   * <p>This method also ensures that any {@code null} custom objects encountered during decoding
   * are instantiated before proceeding with recursive decoding.
   *
   * @param obj the object where to put the decoded fields. This object must not be {@code null} but
   *     values don't matter.
   * @param cache the {@code FieldCache} containing cached fields information for the object's
   *     class. This cache provides the mapping of fields that need to be decoded. Must not be
   *     {@code null}.
   * @param dec the {@code Decoder} responsible for decoding key-value pairs and passing them into
   *     the field updates. Must not be {@code null}.
   * @throws NullPointerException if any of the arguments are {@code null}.
   * @throws InvalidArgumentException if a field corresponding to a key is not found in the class's
   *     metadata.
   * @throws RuntimeException if reflection fails during field access or instantiation (e.g., when
   *     creating a new instance of a custom class).
   */
        ...
        (key, vd) -> {
          ...
          try {
            ...
            throw new InvalidArgumentException(
                "Exception occurred in the java reflections operation", e);
          }
          return true;
        });
  }
  ...
  private static FieldInfo findFieldByName(List<FieldInfo> fields, String name) {
    ...
    int index =
        Collections.binarySearch(
            fields,
            new FieldInfo(name, false, null, null),
            Comparator.comparing(FieldInfo::getName));
    if (index >= 0) {
  ...
  public static String stringifyMapKey(Object key) throws IllegalArgumentException {
    ...
    throw new IllegalArgumentException(
        "Cannot encode key " + key + " of type " + key.getClass().getName());
  }
   ...
   * @param decoder the decoder used to decode the value. Every Decoder is supposed to hold the
   *     value to be decoded in their own way. For example: ListDecoder can have the List/Arrays and
   *     the MapDecoder can have the Map to save the value to be decoded.
   */
