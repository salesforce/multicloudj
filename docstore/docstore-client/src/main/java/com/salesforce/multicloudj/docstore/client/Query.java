 ...
 *
 * <p>This class allows you to construct complex queries with filtering, sorting, pagination, and
 * field selection capabilities across multiple cloud providers including AWS DynamoDB, GCP
 * Firestore, and Alibaba TableStore.
 *
 * <p>The Query class supports:
 *
 * <ul>
 *   <li>Filtering with various operations (EQUAL, GREATER_THAN, LESS_THAN, IN, etc.)
 *   <li>Sorting by field with ascending/descending order
 *   <li>Pagination with offset and limit
 *   <li>Field projection to retrieve only specific fields
 *   <li>Pre-query callbacks for custom processing, not supported yet
 * </ul>
 *
 * <p>Usage example:
 *
 ...
 * </pre>
 *
 * <p><strong>Note:</strong> Multiple filters are combined using AND logic. When using orderBy, the
 * order field must appear in at least one where clause.
 *
 * @since 0.1.0
 */
@Getter
public class Query {
  ...
  /**
   * A list of field paths to retrieve. Each field path is a dot-separated string (e.g.,
   * "user.name", "address.city").
   */
  @Setter private List<String> fieldPaths = new ArrayList<>();

  /**
   * Filters contain a list of filters for the query. If there are more than one filter, they are
   * combined with AND logic.
   */
  private final List<Filter> filters = new ArrayList<>();

  /**
   * Offset (also commonly referred to as `Skip`) sets the number of results to skip before
   * returning results. When offset is less than or equal to 0, the driver implementation returns
   * all possible results from the beginning.
   */
  private int offset = 0;

  /**
   * Limit sets the maximum number of results returned by running the query. When limit is less than
   * or equal to 0, the driver implementation returns all possible results.
   */
  private int limit = 0;

  /**
   * paginationToken sets the pagination token returned by the previous query. Pagination token
   * being empty guarantees that there are no more documents left. However, pagination token being
   * non-empty doesn't guarantee that there are documents left. For example: firestore pagination is
   * stateless cursor values and can only be inferred from the last returned document unless
   * dynamoDB which returns the null lastEvaluatedKey if no more data left.
   */
  private PaginationToken paginationToken = null;

  /**
   * The field to use for sorting the results. Must appear in at least one where clause. If there is
   * a range comparison, it must have the order by clause to work without issues. see: <a
   * href="https://firebase.google.com/docs/firestore/query-data/order-limit-data?limitations#limitations">
   * </a> The order by field also must appear in the cursor <a
   * href="https://firebase.google.com/docs/firestore/reference/rest/v1beta1/StructuredQuery">...</a>
   */
  ...
  /**
   * BeforeQuery is a callback that is called exactly once before the underlying service's query is
   * executed. This allows drivers to expose driver-specific functionality.
   */
   ...
   */
  public Query(AbstractDocStore docStore) {
   ...
   *
   * <ul>
   *   <li>EQUAL - exact match for strings, numbers
   *   <li>GREATER_THAN, LESS_THAN - comparison for numbers and strings
   *   <li>GREATER_THAN_OR_EQUAL_TO, LESS_THAN_OR_EQUAL_TO - inclusive comparisons
   *   <li>IN, NOT_IN - membership testing with collections
   * </ul>
   *
   ...
   * @throws IllegalArgumentException if fieldPath is invalid, operation is unsupported, or value
   *     type is incompatible with the operation
   */
  ...
  /**
   * Checks if the filter value is of a supported type. Supported types are Number and String.
   *
  ...
  /**
   * Sets the number of results to skip before returning results. This is useful for pagination when
   * combined with limit().
   *
   * <p>When offset is 0 or negative, no results are skipped. Only one offset clause is allowed per
   * query.
   *
  ...
  /**
   * Sets the maximum number of results to return. This is useful for pagination and performance
   * optimization.
   *
   * <p>When limit is 0 or negative, all possible results are returned. Only one limit clause is
   * allowed per query.
   *
   ...
   *
   * <p>Use pagination token will make the query start directly from the result of the last query.
   *
   ...
   *
   * <p><strong>Important:</strong> The orderBy field must appear in at least one where clause to
   * ensure efficient query execution across different cloud providers. Only one orderBy clause is
   * allowed per query. If there are in-equality filters, the order-by is advisable to have the
   * consistent results across substrates because of Google cloud firestore limitations. <a
   * href="https://firebase.google.com/docs/firestore/query-data/order-limit-data?limitations#limitations">...</a>
   *
  ...
  /**
   * Sets a callback function to be executed before the query runs. This allows for driver-specific
   * customizations and advanced query modifications.
   *
   * <p>The callback receives a Predicate that can be used to test for driver-specific types and
   * capabilities, enabling conditional logic based on the underlying provider.
   *
   ...
   *
   * <p>This method applies all the configured filters, sorting, pagination, and field selection to
   * retrieve documents from the document store. The results are returned as a DocumentIterator that
   * allows for efficient iteration over potentially large result sets.
   *
   * <p>If specific field paths are provided, only those fields will be retrieved from each
   * document. Field paths support dot notation for accessing nested fields (e.g.,
   * "user.profile.name").
   *
   ...
   *
   * <ul>
   *   <li>Ensures the document store is not closed
   *   <li>Validates all field paths are properly formatted
   *   <li>Ensures orderBy field appears in at least one where clause (if both are specified)
   * </ul>
   *
    ...
    if (orderByField != null && !filters.isEmpty()) {
      ...
      if (!found) {
        throw new IllegalArgumentException(
            String.format("OrderBy field %s must appear in a Where clause", orderByField));
      }
    }
  }
}
