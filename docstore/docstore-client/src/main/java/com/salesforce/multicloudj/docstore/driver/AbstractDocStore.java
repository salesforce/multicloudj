 ...
 * common (i.e. substrate-agnostic) functionality. such as validation
 */
public abstract class AbstractDocStore implements Provider, Collection, AutoCloseable {
  ...
  protected AbstractDocStore(Builder<?, ?> builder) {
    this(
        ...
        builder.region,
        builder.instanceId,
        builder.endpointType,
        builder.endpoint,
        builder.collectionOptions,
        builder.credentialsOverrider);
    if (collectionOptions == null || collectionOptions.getMaxOutstandingActionRPCs() < 1) {
      executorService = Executors.newCachedThreadPool();
    } else {
      executorService =
          Executors.newFixedThreadPool(collectionOptions.getMaxOutstandingActionRPCs());
    }
  }

  protected AbstractDocStore(
      String providerId,
  ...
  protected void toDriverAction(Action action) {
    ...
    if (key == null && action.getKind() != ActionKind.ACTION_KIND_CREATE) {
      throw new IllegalArgumentException(
          "Document key should not be null: " + action.getDocument().toString());
    }
    ...
    if (action.getKind() == ActionKind.ACTION_KIND_CREATE && revision != null) {
      throw new IllegalArgumentException(
          "cannot create a document with a revision field: " + action.getDocument().toString());
    }
  ...
  public void runGets(List<Action> actions, Consumer<Predicate<Object>> beforeDo, int batchSize)
  ...
  private void waitTermination(List<Future<?>> futures)
      throws ExecutionException, InterruptedException {
  ...
  public abstract static class Builder<A extends AbstractDocStore, T extends Builder<A, T>>
      implements Provider.Builder {

    private String providerId;

    /** -- GETTER -- Gets the collection */
    @Getter private Collection collection;

    /** -- GETTER -- Gets the region. */
    @Getter private String region;

    @Getter private String instanceId;

    /** -- GETTER -- Gets the endpoint. */
    @Getter private URI endpoint;

    /** -- GETTER -- Gets the endpoint type. */
    @Getter private String endpointType;

    /** -- GETTER -- Gets the CredentialsOverrider. */
    @Getter private CredentialsOverrider credentialsOverrider = null;
     ...
     */
    @Getter private CollectionOptions collectionOptions = null;
     ...
     *
     * @param collectionOptions collectionOptions Method to supply CollectionOptions
     * @return An instance of self
     */
