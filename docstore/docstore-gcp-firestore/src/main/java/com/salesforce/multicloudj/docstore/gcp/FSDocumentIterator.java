...
/**
 * Iterator for Firestore query results. This implementation properly manages the gRPC stream
 * lifecycle to prevent thread leaks.
 */
public class FSDocumentIterator implements DocumentIterator {
   ...
   * {@inheritDoc}
   *
   * <p>Gets the next document from the stream and populates the provided document.
   *
  ...
  /**
   * Updates the pagination token with cursor values from the last document. This should only be
   * called when we're done iterating or at the limit. The pagination cursor should always include
   * the field of order by clause in order for it work correctly.
   *
  ...
  private void updatePaginationToken(Document document) {
    ...
    String fullDocumentPath =
        String.format(
            "%s/documents/%s/%s", docStore.getDatabasePath(), docStore.getCollectionName(), docId);

    // Create a ReferenceValue for the document key
    Value referenceValue = Value.newBuilder().setReferenceValue(fullDocumentPath).build();
   ...
   * {@inheritDoc}
   *
   * <p>Checks if there are more documents available by looking ahead in the stream.
   *
  ...
  private RunQueryResponse getNextResponseInternal() {
    try {
      ...
      if (e.getStatusCode() != null
          && e.getStatusCode().getCode() == StatusCode.Code.FAILED_PRECONDITION) {
        if (e.getMessage() != null && e.getMessage().contains("The query requires an index.")) {
          throw new com.salesforce.multicloudj.common.exceptions.FailedPreconditionException(
              "The query requires an index.", e);
        }
      }
      throw e;
    }
   ...
   * {@inheritDoc}
   *
   * <p>Stops iteration and releases resources.
   */
  @Override
  public void stop() {
    markDoneAndClose();
