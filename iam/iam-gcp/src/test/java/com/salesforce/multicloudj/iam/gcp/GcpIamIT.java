 ...
 *
 * <p>NOTE: GCP IAM Admin API is gRPC-only. Unlike other conformance tests that use HTTP/HTTPS
 * proxy-based WireMock recording, this test uses:
 *
 * <ul>
 *   <li><b>RECORD mode:</b> Wraps IAMClient to intercept and save responses as JSON files
 *   <li><b>REPLAY mode:</b> Loads saved responses and registers them as WireMock gRPC stubs
 * </ul>
 *
...
public class GcpIamIT extends AbstractIamIT {
  ...
  /**
   * Sets up WireMock server with gRPC extension for replay mode. Synchronized to prevent race
   * conditions if called from multiple test instances.
   */
  private static synchronized void setupGrpcWireMock(int port) throws IOException {
    ...
    WireMockConfiguration config =
        WireMockConfiguration.wireMockConfig().port(port).extensions(new GrpcExtensionFactory());
  ...
  public static class HarnessImpl implements AbstractIamIT.Harness {
    ...
    private static final String TRUSTED_PRINCIPAL =
    ...
    public AbstractIam createIamDriver() {
      ...
      ProjectsSettings.Builder projectsSettingsBuilder =
          ProjectsSettings.newBuilder().setTransportChannelProvider(channelProvider);
      try {
        if (isRecordMode) {

          IAMStubSettings.Builder stubSettingsBuilder = IAMStubSettings.newBuilder();
          stubSettingsBuilder.setTransportChannelProvider(
              getGrpcTransportChannelWithRecordingInterceptor(
                  RECORDINGS_DIR, getCurrentTestPrefix()));
          ...
          projectsSettingsBuilder.setCredentialsProvider(
              FixedCredentialsProvider.create(mockCreds));
          projectsClient = ProjectsClient.create(projectsSettingsBuilder.build());
          return new GcpIam.Builder()
              .withProjectsClient(projectsClient)
              .withIamClient(iamClient)
