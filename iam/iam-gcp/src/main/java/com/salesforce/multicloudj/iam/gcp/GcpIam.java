...
public class GcpIam extends AbstractIam {
  ...
  protected String doCreateIdentity(CreateIdentityRequest request) {
    ...
    if (request.getTrustConfig().isPresent()
      ...
      GetIamPolicyRequest getRequest =
          GetIamPolicyRequest.newBuilder().setResource(serviceAccountResourceName).build();
      ...
      for (String principal : request.getTrustConfig().get().getTrustedPrincipals()) {
        // Format principal as a GCP member (e.g.,
  ...
  /**
   * Formats a principal identifier as a GCP IAM member string. If the principal is already in the
   * correct format (e.g., "serviceAccount:email@..."), returns it as-is. Otherwise, assumes it's a
   * service account email and formats it.
   *
  ...
  protected void doAttachInlinePolicy(AttachInlinePolicyRequest request) {
    ...
    GetIamPolicyRequest getRequest = GetIamPolicyRequest.newBuilder().setResource(resource).build();
  ...
  /**
   * Adds a binding to the IAM policy, merging with existing bindings for the same role. This
   * prevents policy bloat and avoids hitting GCP's IAM policy size limits.
   *
  ...
  private Policy addBinding(Policy policy, String role, String member) {
    ...
    Optional<Binding> existingBinding =
    ...
    if (existingBinding.isPresent()) {
      ...
      if (!binding.getMembersList().contains(member)) {
        ...
        return policy.toBuilder().clearBindings().addAllBindings(updatedBindings).build();
      }
  ...
  /**
   * Retrieves the details of a specific inline policy attached to an IAM member. In GCP, this
   * retrieves the role binding information and converts it back to a PolicyDocument format.
   *
   * <p><strong>GCP-specific behavior:</strong>
   *
   * <ul>
   *   <li><strong>policyName:</strong> Not used. GCP IAM does not support named policies. This
   *       parameter is ignored but kept for interface compatibility.
   *   <li><strong>roleName:</strong> Required. Must be a valid GCP IAM role name (e.g.,
   *       "roles/iam.serviceAccountUser", "roles/storage.objectViewer"). This is used to identify
   *       the role binding to retrieve.
   * </ul>
   *
   * @param request the request containing relevant fields from identity name, policy name, role
   *     name, tenant ID, and region
   *     <p>NOTE on param contents identityName the IAM member (e.g.,
   *     "serviceAccount:my-sa@project.iam.gserviceaccount.com", "user:user@example.com",
   *     "group:group@example.com") policyName the name of the policy. This parameter is optional
   *     and subject to cloud semantics. In GCP, named policies are not supported, so this parameter
   *     is not used but kept for interface compatibility. roleName the role name. This parameter is
   *     optional and subject to cloud semantics. In GCP, this parameter is required and must be a
   *     valid GCP IAM role name (e.g., "roles/iam.serviceAccountUser"). Must not be null or empty.
  ...
  protected String doGetInlinePolicyDetails(GetInlinePolicyDetailsRequest request) {
    ...
    // Get the current IAM policy for the resource
    GetIamPolicyRequest getRequest =
        GetIamPolicyRequest.newBuilder().setResource(request.getTenantId()).build();
    Policy policy = projectsClient.getIamPolicy(getRequest);
    ...
    if (!binding.isPresent()
        || !binding.get().getMembersList().contains(request.getIdentityName())) {
      return null;
    }
  ...
  /**
   * Converts a PolicyDocument to its JSON string representation. Uses Jackson to serialize the
   * document directly.
   *
  ...
  /**
   * Retrieves all policies (roles) attached to an IAM member. In GCP, "attached policies" are the
   * IAM roles that have been granted to the IAM member through bindings in the resource's IAM
   * policy.
   *
  ...
  protected void doRemovePolicy(RemovePolicyRequest request) {
    ...
    if (!existingBinding.isPresent()
        || !existingBinding.get().getMembersList().contains(request.getIdentityName())) {
    ...
    Policy updatedPolicy =
        removeBinding(policy, request.getPolicyName(), request.getIdentityName());
  ...
  /**
   * Removes a member from a role binding in the IAM policy. If the binding becomes empty after
   * removal, the entire binding is removed.
   *
   * @param policy the current IAM policy
  ...
  private Policy removeBinding(Policy policy, String role, String member) {
    ...
    Binding updatedBinding =
        binding.toBuilder().clearMembers().addAllMembers(updatedMembers).build();
   ...
   *
   * <p>This method permanently removes a service account and all its associated IAM bindings. The
  ...
  protected void doDeleteIdentity(DeleteIdentityRequest request) {
    // Build the project resource name in the format "projects/{project-id}"
    String serviceAccountResourceName =
        getServiceAccountResourceName(request.getIdentityName(), request.getTenantId());

    // Delete the service account
    DeleteServiceAccountRequest deleteRequest =
        DeleteServiceAccountRequest.newBuilder().setName(serviceAccountResourceName).build();
   ...
   *
   * <p>This method fetches details of an existing service account and returns its email address as
   ...
   * @throws ApiException if the service account is not found or access is denied (propagates to
   *     IamClient)
   */
  @Override
  protected String doGetIdentity(GetIdentityRequest request) {
    ServiceAccount serviceAccount =
        this.getServiceAccount(request.getIdentityName(), request.getTenantId());
  ...
  private ServiceAccount getServiceAccount(String identityName, String tenantId) {
    ...
    GetServiceAccountRequest getRequest =
        GetServiceAccountRequest.newBuilder().setName(serviceAccountResourceName).build();
  ...
  private static String getServiceAccountResourceName(String identityName, String tenantId) {
    ...
    // Format:
    // projects/{project-id}/serviceAccounts/{account-id}@{project-id}.iam.gserviceaccount.com
