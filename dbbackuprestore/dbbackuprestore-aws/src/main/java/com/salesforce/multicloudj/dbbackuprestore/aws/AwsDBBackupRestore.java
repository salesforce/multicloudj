...
/**
 * AWS implementation of database backup and restore operations using AWS Backup service. This
 * implementation works with DynamoDB tables.
 *
 * @since 0.2.25
 */
...
public class AwsDBBackupRestore extends AbstractDBBackupRestore {
  ...
  public List<Backup> listBackups() {
    ListRecoveryPointsByResourceRequest request =
        ListRecoveryPointsByResourceRequest.builder().resourceArn(tableArn).build();
  ...
  public Restore getRestoreJob(String restoreId) {
    DescribeRestoreJobRequest request =
        DescribeRestoreJobRequest.builder().restoreJobId(restoreId).build();
  ...
  /**
   * Gets the vault name for a recovery point by listing recovery points for this resource. AWS
   * Backup requires both vault name and recovery point ARN, but the ARN alone doesn't contain vault
   * info.
   */
  ...
  public static class Builder extends AbstractDBBackupRestore.Builder<AwsDBBackupRestore, Builder> {
     ...
     * @return this builder
     */
    public Builder withBackupClient(
        software.amazon.awssdk.services.backup.BackupClient backupClient) {
