...
public class DBBackupRestoreClient implements AutoCloseable {
  ...
  private static Iterable<AbstractDBBackupRestore> all() {
    ServiceLoader<AbstractDBBackupRestore> services =
        ServiceLoader.load(AbstractDBBackupRestore.class);
    ...
    try {
      ...
      for (Class<?> innerClass : innerClasses) {
        if (AbstractDBBackupRestore.Builder.class.isAssignableFrom(innerClass)) {
          return (AbstractDBBackupRestore.Builder<?, ?>)
              innerClass.getDeclaredConstructor().newInstance();
        }
      }
      throw new RuntimeException("No Builder class found in provider: " + providerClass.getName());
    } catch (Exception e) {
  ...
  /**
   * Restores a table from a backup. The restore operation may take time depending on the backup
   * size and is async request.
   *
