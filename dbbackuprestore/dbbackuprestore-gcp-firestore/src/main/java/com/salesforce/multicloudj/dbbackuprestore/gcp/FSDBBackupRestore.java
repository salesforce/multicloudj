...
/**
 * GCP Firestore implementation of database backup and restore operations. This implementation uses
 * the Firestore Admin API for backup/restore operations.
 *
 * @since 0.2.25
 */
...
public class FSDBBackupRestore extends AbstractDBBackupRestore {
  ...
  public String restoreBackup(RestoreRequest request) {
    ...
    // Extract parent from backup ID (format:
    // projects/{project}/locations/{location}/backups/{backup})
    String parent =
        request.getBackupId().substring(0, request.getBackupId().lastIndexOf("/locations/"));

    RestoreDatabaseRequest.Builder restoreBuilder =
    ...
    return Backup.builder()
        ...
        .creationTime(
            Instant.ofEpochSecond(
                backup.getSnapshotTime().getSeconds(), backup.getSnapshotTime().getNanos()))
        .expiryTime(
            Instant.ofEpochSecond(
    ...
    if (operation.hasMetadata()) {
      try {
        ...
        if (metadata.hasEndTime()) {
          endTime =
              Instant.ofEpochSecond(
                  metadata.getEndTime().getSeconds(), metadata.getEndTime().getNanos());
        }
        if (metadata.hasStartTime()) {
          startTime =
              Instant.ofEpochSecond(
                  metadata.getStartTime().getSeconds(), metadata.getStartTime().getNanos());
        }
