...
/**
 * Alibaba Cloud implementation of database backup and restore operations. This implementation uses
 * the HBR (Hybrid Backup Recovery) service for TableStore backups.
 *
 * <p>The HBR service manages TableStore backups through OTS (Open Table Service) snapshots. Backups
 * are created by HBR backup plans and can be restored using the CreateRestoreJob API.
 *
 * @since 0.2.25
 */
...
public class AliDBBackupRestore extends AbstractDBBackupRestore {
  ...
  public List<Backup> listBackups() {
    ...
    for (DescribeOtsTableSnapshotsResponseBody.DescribeOtsTableSnapshotsResponseBodySnapshots
        snapshot : response.getBody().getSnapshots()) {
  ...
  public Restore getRestoreJob(String restoreId) {
    ...
    try {
      ...
      if (response.getBody() == null
          || response.getBody().getRestoreJobs() == null
      ...
      DescribeRestoreJobs2ResponseBody.DescribeRestoreJobs2ResponseBodyRestoreJobsRestoreJob
          restoreJob = response.getBody().getRestoreJobs().getRestoreJob().get(0);
  ...
  private Backup convertToBackup(
      DescribeOtsTableSnapshotsResponseBody.DescribeOtsTableSnapshotsResponseBodySnapshots
          snapshot) {
   ...
   *
   * @param status the Alibaba snapshot status, valid statuses are here <a
   *     href="https://www.alibabacloud.com/help/en/cloud-backup/developer-reference/api-hbr-2017-09-08-describebackupjobs2">...</a>
   ...
   */
  private Restore convertToRestore(
      DescribeRestoreJobs2ResponseBody.DescribeRestoreJobs2ResponseBodyRestoreJobsRestoreJob
          restoreJob) {
