 ...
 * This example shows how to use the multicloudj library to pull images from cloud container registries.
 *
 * Usage: java -jar registry-example.jar [provider]
 *   - provider: Cloud provider (gcp, aws) - defaults to "gcp"
 *
 * Examples:
 *   java -jar registry-example.jar
 *   java -jar registry-example.jar gcp
 *   java -jar registry-example.jar aws
 */
public class Main {
    ...
    private static void printWelcomeBanner() {
        System.out.println();
        System.out.println("╔══════════════════════════════════════════════════════════════════════════════╗");
        System.out.println("║           🚀 MultiCloudJ Container Registry Demo 🚀                        ║");
        System.out.println("║             Cross-Cloud Container Registry Operations                       ║");
        System.out.println("╚══════════════════════════════════════════════════════════════════════════════╝");
        System.out.println();
    }
    ...
    /**
     * Demonstrates extracting the full image filesystem via {@link ContainerRegistryClient#extract(Image)}.
     */
    ...
    private void demonstrateErrorHandling() {
        ...
        try (ContainerRegistryClient client = initializeClient()) {
            ...
            showSuccess("Correctly caught ResourceNotFoundException: "
                    + truncate(e.getMessage(), 100));
        } catch (SubstrateSdkException e) {
            showError("Unexpected SDK exception: " + e.getClass().getSimpleName()
                    + " - " + e.getMessage());
        } catch (Exception e) {
            showError("Unexpected error: " + e.getMessage());
